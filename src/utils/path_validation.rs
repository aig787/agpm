...
pub fn validate_skill_installation_path(
    ...
    // Reject Unix-style absolute paths (starting with '/') for cross-platform compatibility
    if installed_at.starts_with('/') {
        return Err(anyhow!(
    ...
    let full_path = if path.is_absolute() {
        // Absolute paths are not allowed for skill installation (Windows-style: C:\, \\server\share)
        return Err(anyhow!(
            "Skill installation path must be relative to project directory: '{}'",
            installed_at
        ));
    ...
    // Normalize path separators to forward slashes for consistent pattern matching across platforms
