...
/// Maximum number of files allowed in a skill directory (soft limit with warning)
const MAX_SKILL_FILES: usize = 100;

/// Maximum total size in bytes for all files in a skill (soft limit with warning)
const MAX_SKILL_SIZE_BYTES: u64 = 10 * 1024 * 1024; // 10 MB
...
pub fn extract_skill_metadata(skill_path: &Path) -> Result<(SkillFrontmatter, Vec<String>)> {
    ...
    // Validate file count (soft limit - warning only)
    if files.len() > MAX_SKILL_FILES {
        tracing::warn!(
            "Skill '{}' contains {} files, which exceeds recommended limit of {}. \
    ...
    // Validate total size (soft limit - warning only)
    if total_size > MAX_SKILL_SIZE_BYTES {
        let size_mb = total_size as f64 / (1024.0 * 1024.0);
        let limit_mb = MAX_SKILL_SIZE_BYTES as f64 / (1024.0 * 1024.0);
        tracing::warn!(
            "Skill '{}' total size is {:.2} MB, which exceeds recommended limit of {:.0} MB. \
            This may impact performance and installation time.",
            frontmatter.name,
            size_mb,
            limit_mb
        );
