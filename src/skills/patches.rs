...
/// let project_patches = HashMap::new();
/// let private_patches = HashMap::new();
///
...
"#;
        ...
        let (new_content, applied) =
            apply_skill_patches_preview(content, &patches, &HashMap::new()).unwrap();
...
"#;
        ...
        let project_patches = HashMap::from([(
            "model".to_string(),
            toml::Value::String("claude-3-opus".to_string()),
        )]);
        let private_patches =
            HashMap::from([("temperature".to_string(), toml::Value::String("0.7".to_string()))]);
...
"#;
        ...
        let private_patches = HashMap::from([(
            "model".to_string(),
