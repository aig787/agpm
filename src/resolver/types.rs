...
/// Add a dependency to the conflict detector with consistent logic.
///
/// This function consolidates conflict detection logic for both direct and transitive
/// dependencies, ensuring consistent behavior across the resolver.
///
/// # Arguments
///
/// * `conflict_detector` - The conflict detector to add the requirement to
/// * `name` - The name of the dependency
/// * `dep` - The dependency specification
/// * `required_by` - The resource that requires this dependency
pub fn add_dependency_to_conflict_detector(
    ...
    // Skip content-only dependencies (install=false) as they don't create files
    if dep.get_install() == Some(false) {
        tracing::debug!(
            "Skipping conflict detection for content-only dependency '{}' (install=false)",
            name
        );
        return;
    }

    // Build resource identifier using the same logic as direct dependencies
    ...
    conflict_detector.add_requirement(&resource_id, required_by, version);

    tracing::debug!(
        "Added to conflict detector: '{}' (resource_id: {}, version: {}, required_by: {})",
        name,
        resource_id,
        version,
        required_by
...
mod tests {
    ...
    #[test]
    fn test_add_dependency_to_conflict_detector_skips_install_false() {
        let mut detector = ConflictDetector::new();

        // Create a dependency with install=false
        let dep = ResourceDependency::Detailed(Box::new(DetailedDependency {
            source: Some("test-repo".to_string()),
            path: "snippets/test.md".to_string(),
            version: Some("v1.0.0".to_string()),
            install: Some(false), // Content-only
            branch: None,
            rev: None,
            command: None,
            args: None,
        ...
        // Add to conflict detector - should be skipped
        add_dependency_to_conflict_detector(&mut detector, "test", &dep, "parent");
        ...
        assert_eq!(
            conflicts.len(),
            0,
            "install=false deps should not be added to conflict detector"
        );
    }

    #[test]
    fn test_add_dependency_to_conflict_detector_includes_install_true() {
        let mut detector = ConflictDetector::new();

        // Create a dependency with install=true (default)
        let dep = ResourceDependency::Detailed(Box::new(DetailedDependency {
            source: Some("test-repo".to_string()),
            path: "snippets/test.md".to_string(),
            version: Some("v1.0.0".to_string()),
            install: Some(true), // Creates file
            branch: None,
            rev: None,
            command: None,
            args: None,
        ...
        // Add to conflict detector - should be included
        add_dependency_to_conflict_detector(&mut detector, "test", &dep, "parent");

        // Should have one requirement
        let conflicts = detector.detect_conflicts();
        assert_eq!(conflicts.len(), 0, "Single dependency should not conflict");
    }

    #[test]
    fn test_add_dependency_to_conflict_detector_skips_local() {
        let mut detector = ConflictDetector::new();

        // Create a local dependency
        let dep = ResourceDependency::Detailed(Box::new(DetailedDependency {
            source: None, // Local dependency
            path: "snippets/test.md".to_string(),
            version: Some("v1.0.0".to_string()),
            branch: None,
            rev: None,
        ...
        // No conflicts should be detected
        let conflicts = detector.detect_conflicts();
        assert_eq!(conflicts.len(), 0, "Local deps should not be added to conflict detector");
    }

    #[test]
    fn test_add_dependency_to_conflict_detector_skips_install_none() {
        let mut detector = ConflictDetector::new();

        // Create a dependency with install=None (default is true)
        let dep = ResourceDependency::Detailed(Box::new(DetailedDependency {
            source: Some("test-repo".to_string()),
            path: "snippets/test.md".to_string(),
            version: Some("v1.0.0".to_string()),
            install: None, // Default behavior
            branch: None,
            rev: None,
            command: None,
            args: None,
        ...
        assert_eq!(
            conflicts.len(),
            0,
            "install=None (default true) should be added to conflict detector"
        );
    }
