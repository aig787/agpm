...
/// Update .claude/.gitignore with installed file paths
pub fn update_gitignore(lockfile: &LockFile, project_dir: &Path, enabled: bool) -> Result<()> {
    ...
    let gitignore_path = project_dir.join(".claude").join(".gitignore");
    ...
    if gitignore_path.exists() {
        ...
        for line in content.lines() {
            ...
            if !in_ccpm_section && !past_ccpm_section {
                // Lines before CCPM section (header/user entries)
                if line.starts_with('#') || line.trim().is_empty() {
                    header_lines.push(line.to_string());
                } else {
                    user_entries.push(line.to_string());
                }
            } else if in_ccpm_section {
                // Track existing CCPM entries
                if !line.trim().is_empty() && !line.starts_with('#') {
                    existing_entries.insert(line.to_string());
                }
            } else {
                // Lines after CCPM section (additional user entries)
                user_entries.push(line.to_string());
            }
        }
    } else {
        // Default header for new file
        header_lines.push("# .claude/.gitignore - CCPM managed file".to_string());
        header_lines.push("# This file is automatically generated by CCPM".to_string());
    ...
    if !user_entries.is_empty() {
        new_content.push_str("# User entries\n");
    ...
    // Convert paths to gitignore format (relative to .claude directory)
    // Sort paths for consistent output
    let mut sorted_paths: Vec<_> = paths_to_ignore.into_iter().collect();
    sorted_paths.sort();

    for path in sorted_paths {
        // If path starts with .claude/, make it relative to .claude directory
        let ignore_path = if path.starts_with(".claude/") {
            // Remove .claude/ prefix since gitignore is in .claude/
            path.strip_prefix(".claude/").unwrap_or(&path).to_string()
        } else if path.starts_with("./claude/") {
            // Handle variant with leading ./
            path.strip_prefix("./claude/").unwrap_or(&path).to_string()
        } else {
            // For paths outside .claude, use ../ to reference parent
            format!("../{}", path)
        };

        // Add forward slash prefix for paths within .claude
...
mod tests {
    ...
    async fn test_update_gitignore_creates_new_file() {
        ...
        assert!(content.contains("/agents/test-agent.md"));
        assert!(content.contains("/ccpm/snippets/test-snippet.md"));
    }
    ...
    async fn test_update_gitignore_preserves_user_entries() {
        ...
        // Create .claude directory and existing gitignore with user entries
        ...
        assert!(updated_content.contains("/agents/new-agent.md"));

        // Check that old managed entries are replaced
        assert!(!updated_content.contains("/old/entry.md"));
    }
    ...
    async fn test_update_gitignore_handles_external_paths() {
        ...
        assert!(content.contains("/agents/test.md"));
    }
