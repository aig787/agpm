...
"#,
    ...
    // Check that batch processing messages appear in debug output
    // (This is a loose check since exact formatting may vary)
    assert!(
        output.stderr.contains("Processing")
            || output.stderr.contains("parallel")
            || output.stderr.contains("batch")
            || output.stderr.contains("workers"),
        "Should show some evidence of parallel/batch processing in debug output"
    );
