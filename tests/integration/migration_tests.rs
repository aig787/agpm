...
"#,
    ...
    assert!(
        error_output.contains("context_checksum") || error_output.contains("regenerate") || error_output.contains("migrate") || error_output.contains("upgrade"),
...
"#,
    ...
    assert!(
        agent_path.exists(),
        "Templated agent should be installed"
    );
    ...
    assert!(
        ...
        "Lockfile should contain context_checksum"
    );
    assert!(
        lockfile_content.contains("template_vars"),
        "Lockfile should contain template_vars for backward compatibility"
    );
    ...
    assert!(
        ...
        "Content should be rendered with template variables"
    );
...
"#,
    ...
    assert!(
        installable_path.exists(),
        "Installable snippet should be installed"
    );
    assert!(
        content_only_path.exists(),
        "Content-only snippet should be installed initially"
    );
    assert!(
...
"#,
    ...
    assert!(
        installable_path.exists(),
        "Installable snippet should still exist"
    );
    assert!(
        !content_only_path.exists(),
        "Content-only snippet should be removed"
    );
    assert!(
        dependency_path.exists(),
...
"#,
    ...
    // Install
    let output = project.run_agpm(&["install"])?;
    assert!(output.success, "Install should succeed with template_vars alias. Stderr: {}", output.stderr);

    // Verify resource is installed
    let command_path = project.project_path().join(".claude/commands/variant.md");
    assert!(
        command_path.exists(),
        "Variant command should be installed"
    );
    ...
    assert!(
        ...
        "API URL should be rendered"
    );
    assert!(
        content.contains("30s"),
        "Timeout should be rendered"
    );
    assert!(
        content.contains("auth, logging"),
    ...
    assert!(
        lockfile_content.contains("template_vars"),
        "Lockfile should contain template_vars (backward compatibility)"
    );
...
"#,
    ...
    for run in 1..=3 {
        ...
        assert!(
            plain_path.exists(),
            "Plain agent should exist in run {}",
            run
        );
        assert!(
            templated_path.exists(),
            "Templated agent should exist in run {}",
            run
        );
    }

    // All lockfiles should be identical (post-migration consistency)
    for i in 1..lockfiles.len() {
        assert_eq!(
            lockfiles[0], lockfiles[i],
            "Lockfiles should be consistent post-migration. Run 1 vs Run {}:\n\nRun 1:\n{}\n\nRun {}:\n{}",
            i + 1, lockfiles[0], i + 1, lockfiles[i]
        );
    }

    // Verify context checksum is present for templated resource but not for plain
    let lockfile_content = &lockfiles[0];
    assert!(
        lockfile_content.contains("context_checksum"),
        "Lockfile should contain context checksum"
    );

    // Should have exactly one context checksum (only for templated resource)
    let context_checksum_count = lockfile_content.matches("context_checksum").count();
    assert_eq!(
        context_checksum_count, 1,
        "Should have exactly one context checksum (for templated resource), found {}",
        context_checksum_count
    );
