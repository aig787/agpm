...
/// Test validating manifest with unsupported resource type for artifact type
#[tokio::test]
async fn test_validate_unsupported_resource_type() {
    let project = TestProject::new().await.unwrap();

    // Create manifest with snippets using opencode artifact type (not supported)
    let manifest_content = r#"
[sources]
...
"#;
    ...
    assert!(
        output
            .stdout
            .contains("Resource type 'snippets' is not supported by artifact type 'opencode'")
    );
