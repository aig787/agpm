...
mod tests {
    ...
    #[test]
    fn test_claude_code_tool_configuration() {
        let manifest = ManifestBuilder::new().with_claude_code_tool().build();

        assert!(manifest.contains("[tools]"));
        assert!(manifest.contains("[tools.claude-code]"));
        assert!(manifest.contains("path = \".claude\""));
        assert!(manifest.contains("[tools.claude-code.resources]"));
        assert!(manifest.contains("agents = { path = \"agents\" }"));
        assert!(manifest.contains("hooks = { merge-target = \".claude/settings.local.json\" }"));
        assert!(manifest.contains("mcp-servers = { merge-target = \".mcp.json\" }"));
    }

    #[test]
    fn test_opencode_tool_configuration() {
        let manifest = ManifestBuilder::new().with_opencode_tool().build();

        assert!(manifest.contains("[tools]"));
        assert!(manifest.contains("[tools.opencode]"));
        assert!(manifest.contains("path = \".opencode\""));
        assert!(manifest.contains("enabled = false"));
        assert!(manifest.contains("agents = { path = \"agent\" }"));
        assert!(manifest.contains("mcp-servers = { merge-target = \".opencode/opencode.json\" }"));
    }

    #[test]
    fn test_malformed_hooks_tool_configuration() {
        let manifest = ManifestBuilder::new().with_malformed_hooks_tool().build();

        assert!(manifest.contains("[tools]"));
        assert!(manifest.contains("[tools.claude-code]"));
        assert!(manifest.contains("hooks = {  }"));
        // Should have empty hooks configuration (neither path nor merge-target)
        assert!(!manifest.contains("hooks = { path ="));
        assert!(!manifest.contains("hooks = { merge-target ="));
    }

    #[test]
    fn test_missing_hooks_tool_configuration() {
        let manifest = ManifestBuilder::new().with_missing_hooks_tool().build();

        assert!(manifest.contains("[tools]"));
        assert!(manifest.contains("[tools.claude-code]"));
        assert!(manifest.contains("agents = { path = \"agents\" }"));
        assert!(manifest.contains("mcp-servers = { merge-target = \".mcp.json\" }"));
        // Should NOT contain hooks configuration at all
        assert!(!manifest.contains("hooks = {"));
    }

    #[test]
    fn test_empty_hooks_tool_configuration() {
        let manifest = ManifestBuilder::new().with_empty_hooks_tool().build();

        assert!(manifest.contains("[tools]"));
        assert!(manifest.contains("[tools.claude-code]"));
        assert!(manifest.contains("hooks = {  }"));
        // Should have empty hooks configuration
        assert!(manifest.contains("hooks = {  }"));
    }

    #[test]
    fn test_custom_tools_configuration() {
        let manifest = ManifestBuilder::new()
            .with_tools_config(|t| {
                t.tool("custom-tool", |c| {
                    c.path(".my-tool")
                        .enabled(true)
                        .agents(ResourceConfigBuilder::default().path("agents").flatten(true))
                        .hooks(ResourceConfigBuilder::default().merge_target(".my-tool/hooks.json"))
                })
            })
            .build();

        assert!(manifest.contains("[tools]"));
        assert!(manifest.contains("[tools.custom-tool]"));
        assert!(manifest.contains("path = \".my-tool\""));
        assert!(manifest.contains("enabled = true"));
        assert!(manifest.contains("agents = { path = \"agents\", flatten = true }"));
        assert!(manifest.contains("hooks = { merge-target = \".my-tool/hooks.json\" }"));
    }

    #[test]
    fn test_complete_manifest_with_tools_and_dependencies() {
        let manifest = ManifestBuilder::new()
            .add_source("official", "file:///official.git")
            .with_claude_code_tool()
            .add_standard_agent("my-agent", "official", "agents/my-agent.md")
            .add_hook("my-hook", |d| {
                d.source("official").path("hooks/my-hook.json").version("v1.0.0")
            })
            .build();
        ...
        assert!(manifest.contains("[tools.claude-code]"));
        assert!(manifest.contains("merge-target = \".claude/settings.local.json\""));

        // Should contain dependencies
        assert!(manifest.contains("my-agent = {"));
        assert!(manifest.contains("my-hook = {"));
    }
}
